# PlayneraTest
## Unity 2022.3.54f1 / 3rd party assets: Zenject, UniRx

# Описание: 
#### Для написания тестового использолся компонентный подход. В качесте Di фреймворка Zenject
  
Сцена игры: Assets/ Game/ Scenes/ GameScene
  
Реализованные механики: перемещение объектов по сцене, скролл камеры.  
На сцене есть `SceneInstaller`, в котором биндятся все зависимости на сцене. Также есть `GameObjectInstaller` для биндинга всех компонентов Яблока: `AppleInstaller`.  
`IObjFacade` - по большей части фасад для игровых объектов на сцене.  
